...
theorem exists_minimal (em : ∀ p : Prop, p ∨ ¬p) (x : PSet.{u}) :
    ∀ z, z ∈ x → ∃ y, y ∈ x ∧ ∀ w, w ∈ y → ¬ w ∈ x := by
  intro z
  induction z using mem_induction with | _ z ih => ?_
  intro hz
  rcases em (∃ w, w ∈ z ∧ w ∈ x) with ⟨w, hw, hwx⟩ | h
  · exact ih w hw hwx
  · exact ⟨z, hz, fun w hw hwx => h ⟨w, hw, hwx⟩⟩

namespace ZFModel
variable {M : PSet.{u} → Prop} (hM : ZFModel M) (em : ∀ p : Prop, p ∨ ¬p)
include hM em
...
theorem con : Con ZF := Con.of_model em (hM.valid em) PSet.empty hM.empty
