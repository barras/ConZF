...
Lemma IsNat.resp {t t' : PSet} (h : IsNat t) (e : t ≈ t') : IsNat t' := by
  cases h with
  | zero h => exact .zero (e.symm.trans h)
  | succ h h' => exact .succ h (e.symm.trans h')

Lemma IsNat.pred {t t' : PSet} (h : IsNat t) (e : t ≈ PSet.succ t') : IsNat t' := by
  cases h with
  | zero h =>
...
inductive Chain (η : PSet) : Nat -> PSet -> Prop
  | zero {t} : η ≈ t -> Chain η 0 t
  | succ {k t t'} : Chain η k t -> t ≈ succ t' -> Chain η (k+1) t'

Lemma Chain.func {η : PSet} {k t t'} (h : Chain η k t) (h' : Chain η k t') : t ≈ t' := by
...
Lemma Chain.resp {η : PSet} {k t t'} (h : Chain η k t) (e : t ≈ t') : Chain η k t' := by
  cases h with
  | zero h => exact .zero (h.trans e)
  | succ h h' => exact .succ h (h'.trans (equiv_iff_mem.2 fun _ => mem_succ_congr e))

Lemma Chain.resp_left {η η' : PSet} {k t} (e : η ≈ η') (h : Chain η k t) : Chain η' k t := by
  induction h with
  | zero h => exact .zero (e.symm.trans h)
  | succ _ h' ih => exact .succ ih h'

Lemma Chain.isNat {η : PSet} {k t} (h : Chain η k t) (hη : IsNat η) : IsNat t := by
  induction h with
...
predecessor of `η`. *)
Definition NatT (η : PSet) (p : Path) (t : PSet) : Prop :=
  exists k, p = List.replicate k .a /\ Chain η k t

Lemma natT_cons {η : PSet} {l p t} (h : NatT η (l :: p) t) :
    l = .a /\ exists k, p = List.replicate k .a /\ Chain η (k+1) t := by
  obtain ⟨k, e, h⟩ := h
  cases k with
  | zero => cases e
  | succ k =>
...
Lemma natT_coherent (D : PSet -> PSet) (U η : PSet) (hη : IsNat η) :
    Coherent D U (NatT η) /\ NatT η [] η := by
  refine ⟨?_, 0, rfl, .zero (Equiv.refl _)⟩
  have same : forall {k k' : Nat}, List.replicate k Label.a = List.replicate k' .a -> k = k' :=
...
Lemma replacement_nat (D : PSet -> PSet) (s : PSet) (φ : PSet -> PSet -> Prop)
    (φ_resp : forall {x x' y y'}, x ≈ x' -> y ≈ y' -> φ x y -> φ x' y')
    (φ_func : forall {x y y'}, x ∈ s -> φ x y -> φ x y' -> y ≈ y')
    (φ_nat : forall {x y}, x ∈ s -> φ x y -> IsNat y) :
