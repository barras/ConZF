...
a universe are bounded independently of that extent. We prove the rank
lemma, by majorization in Howard's style, for the theory with two
universes and recursion on the natural numbers only, where the extent
of the universe can be taken to be $\omega_1$; in general it is open,
and we say what the general case adds. This is a working paper: it
...
Fix a cut $\alpha$. A code is \emph{pure} if the universe constant does
not occur in it: the constants other than $\code{\U_0}$ are pure,
...
\item $e\lhd_o\beta$, for $e$ not a class, iff $e$ is a number or $\star$;
  or $e$ is a pure code whose rank is finite, or at least $\alpha$, or
  below $\beta$; or $e$ is a pair both of whose components are
...
The relation depends on the cut, through the clause at $o$, and on
nothing else: not on the oracle, and not on a code. Profiles do not
mention the cut at all.
...
$\lhd_o\beta'$ by induction, in particular pure. So the code is pure. If
its rank is infinite and below $\alpha$ then so are the ranks of $c$ and
of the fibres that have infinite rank, which are therefore below $\beta$
and $\beta'$; the elements of $\El^\X(c)$ have rank below
...
$\sem t^\X\rho$ is a code of rank below $\alpha$ and at least $\omega$, then
its rank is below $G_{t,\theta}(\gamma)$.
\end{theorem}
\begin{proof}
...
of rank in $[\omega,\alpha)$ has rank below it by the clause at $o$. If
...
\emph{Proved in a special case:} the rank lemma for $\CIC_2$ without $W$
and $\Acc$ (Theorem~\ref{thm:rankcic2}), with $\omega_1$ as the extent of
$\Type_0$ (Corollary~\ref{cor:omega1}); its proof assumes the standard
metatheory of the intensional theory, in Lemma~\ref{lem:rigid}.
\emph{Open:} Conjecture~\ref{conj:rank} in general, on which the theorem
depends; the two things the general case adds are listed at the end of
Section~\ref{sec:proved}, and the next case to try is $\CIC_2$ with $W$ and
$\Acc$, which has the first and not the second.
